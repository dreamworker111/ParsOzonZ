...
def mark_mobile_session_saved() -> None:
    """Record that the user completed the separate mobile login flow."""
    ensure_session_dir()
    MOBILE_SESSION_MARKER.write_text(
        json.dumps({"profile": str(MOBILE_CHROME_PROFILE)}, ensure_ascii=False),
        encoding="utf-8",
    )


def has_mobile_saved_session() -> bool:
    """A mobile auth session is usable only after the login window was closed."""
    return MOBILE_SESSION_MARKER.exists() and _profile_initialized(MOBILE_CHROME_PROFILE)
